...
upd_dish = {"title": "Updated borsh", "description": "brand new borsh", "price": 450}
...
def fixture_submenu(db):
    ...
    submenu = crud.create_submenu(menu.id, schemas.SubmenuCreate(**new_submenu), db)
...
def test_patch_submenu(fixture_submenu, client):
    ...
    response = client.patch(f'/api/v1/menus/{menu_id}/submenus/{submenu_id}', json=upd_submenu)
    assert response.status_code == 200
    assert response.json()["id"] == submenu_id
    assert response.json()['title'] == submenu_title
    assert response.json()['description'] == submenu_description
...
def test_delete_submenu(fixture_submenu, client):
    ...
    assert client.get(f'/api/v1/menus/{menu_id}/submenus/{submenu_id}').status_code == 404
...
def test_post_dish(fixture_submenu, client):
    ...
    dish_price = '{0:0.2f}'.format(new_dish["price"])
...
def test_get_dish(fixture_dish, client):
    ...
    dish_price = '{0:0.2f}'.format(fixture_dish[2].price)
...
def test_get_dish_not_found(fixture_submenu, client):
    ...
    response = client.get(f'/api/v1/menus/{menu_id}/submenus/{submenu_id}/dishes/666')
    assert response.status_code == 404
...
def test_patch_dish(fixture_dish, client):
    ...
    dish_price = '{0:0.2f}'.format(upd_dish["price"])

    response = client.patch(f'/api/v1/menus/{menu_id}/submenus/{submenu_id}/dishes/{dish_id}', json=upd_dish)
    assert response.status_code == 200
    assert response.json()["id"] == dish_id
    assert response.json()['title'] == dish_title
    assert response.json()['description'] == dish_description
...
def test_delete_dish(fixture_dish, client):
    ...
    assert client.get(f'/api/v1/menus/{menu_id}/submenus/{submenu_id}/dishes/{dish_id}').status_code == 404